...
const Beauty = () => {
  ...
  const [allBeauty, setAllBeauty] = useState([]);  // Armazena todos os medicamentos recebidos
  ...
  useEffect(() => {
    ...
    const url = 'http://localhost:3000/Beauty';

    // Fetch data from JSON Server
    axios.get(url)
      .then(response => {
        setAllBeauty(response.data);
        setBeauty(response.data.slice(0, visibleCount));  // Inicialmente mostra os primeiros 3 itens
        setLoading(false);
      })
      .catch(error => {
        setError(error);
  ...
  const handleLoadMore = () => {
    setVisibleCount(prevCount => {
      ...
      setBeauty(allBeauty.slice(0, newCount));  // Atualiza os itens visíveis
      return newCount;
    });
  };
